...
export const sendClaimUpdateSMS = async (to, status) => {
...
export const sendVendorScheduledSMS = async (to, vendor, date) => {
    const body = `HairCabello: A delivery from ${vendor} has been scheduled for ${date}.`;
    console.log(`[SMS] To: ${to}, Body: ${body}`);
};

export default { sendClaimUpdateSMS, sendVendorScheduledSMS };
